...
const ssl_certificates_service_1 = require("../../../src/modules/ssl-certificates/ssl-certificates.service");
...
describe('SslCertificatesService', () => {
    ...
    beforeEach(async () => {
        const module = await testing_1.Test.createTestingModule({
            providers: [
                ssl_certificates_service_1.SslCertificatesService,
                {
                    provide: (0, typeorm_1.getRepositoryToken)(ssl_certificate_entity_1.SslCertificate),
                    useValue: {
                        create: jest.fn(),
                        save: jest.fn(),
                        find: jest.fn(),
        ...
        service = module.get(ssl_certificates_service_1.SslCertificatesService);
        sslCertificateRepository = module.get((0, typeorm_1.getRepositoryToken)(ssl_certificate_entity_1.SslCertificate));
    });
    ...
    describe('create', () => {
        ...
        it('should create a new SSL certificate successfully', async () => {
            ...
            sslCertificateRepository.findOne.mockResolvedValue(null);
            sslCertificateRepository.create.mockReturnValue(newCertificate);
            sslCertificateRepository.save.mockResolvedValue(newCertificate);
    ...
    describe('renewCertificate', () => {
        ...
        it('should throw NotFoundException if certificate not found', async () => {
            sslCertificateRepository.findOne.mockResolvedValue(null);
            await expect(service.renewCertificate('non-existent')).rejects.toThrow(common_1.NotFoundException);
        });
    });
    describe('renewExpiredCertificates', () => {
        it('should renew all expired and expiring certificates', async () => {
            const expiredCerts = [
                { ...mockSslCertificate, id: 'cert-1', status: ssl_certificate_entity_1.CertificateStatus.EXPIRED },
                { ...mockSslCertificate, id: 'cert-2', status: ssl_certificate_entity_1.CertificateStatus.EXPIRING },
            ];
    ...
    describe('getStats', () => {
        it('should return certificate statistics', async () => {
            sslCertificateRepository.count.mockImplementation((options) => {
                if (!options)
                    return Promise.resolve(10);
                if (options.where?.status === 'valid')
                    return Promise.resolve(7);
                if (options.where?.status === 'expired')
...
//# sourceMappingURL=ssl-certificates.service.spec.js.map